...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    """
    # TODO: Implement function
    # Conv 1x1 for vgg layer7, layer4, layer3
    ...
    # Upsample of vgg layer7 conv 1x1
    layer7_upsample = tf.layers.conv2d_transpose(layer7_conv1x1, num_classes, 4,
                                                 strides=(2,2),
                                                 padding="same",
                                                 kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Skip connection for vgg layer4 conv 1x1
    layer4_skip = tf.add(layer7_upsample, layer4_conv1x1)

    # Upsample of vgg layer4 skip connection
    layer4_upsample = tf.layers.conv2d_transpose(layer4_skip, num_classes, 4,
                                                 strides=(2, 2),
                                                 padding="same",
                                                 kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    # Skip connection for vgg layer3 conv 1x1
    layer3_skip = tf.add(layer4_upsample, layer3_conv1x1)

    # Upsample of vgg layer3 skip connection
